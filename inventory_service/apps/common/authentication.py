# inventory_service/apps/common/authentication.py
...
class JWTAuthenticationNoDB(authentication.BaseAuthentication):
    def authenticate(self, request):
        ...
        try:
            ...
            validated = token_backend.decode(token_str, verify=True)
        except Exception as e:
            raise exceptions.AuthenticationFailed('Invalid or expired token.') from e
        user_id = validated.get(settings.SIMPLE_JWT.get('USER_ID_CLAIM','user_id')) or validated.get('user_id')
        username = validated.get('username') or validated.get('email')
        role = validated.get('role')
        permissions = validated.get('permissions', [])
        user = SimpleNamespace(is_authenticated=True, id=user_id, username=username, role=role, permissions=permissions, is_superuser=validated.get('is_superuser', False))
        request._auth = validated
