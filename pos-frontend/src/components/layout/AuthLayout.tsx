...
const chooseTheme = (main: string, timeOfDay: string): AuthLayoutTheme => {
  ...
  if (main === "Clear") {
    ...
    if (timeOfDay === "morning") {
      return {
        ...
        button: "bg-yellow-600 text-white hover:bg-yellow-700 focus:ring-yellow-300",
      };
    }
    return {
  ...
  return {
    ...
    button: "bg-indigo-600 text-white hover:bg-indigo-700 focus:ring-indigo-300",
  };
};
...
export default function AuthLayout({ children, showWeatherInfo = true }: AuthLayoutProps) {
  ...
  // Fetch weather data
  const { data: weatherData, error: weatherError, isLoading: weatherLoading } = useSWR<WeatherResponse>(
    apiUrl,
    fetcher,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: theme.background }}>
      ...
      <div className="relative z-10 w-full max-w-md p-6">
        ...
        {/* Main content with render prop */}
        {children({ weatherData, weatherLoading, weatherError, mainWeather, theme })}
      </div>
    </div>
  );
}

// Export hook for using weather data in components
